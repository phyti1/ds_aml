...
plot_confusion_matrix(regression_baseline_predictions_threshold[[id_set]], val_sets[[id_set]]$has_card)
```
...
Nun soll dieses Baseline-Modell verbessert werden.
...
Fast jede Zeile hat irgendwo ein NA.
Es sind auch viele Kolonnen betroffen.
Wir können also nicht alle Observationen oder Variabeln mit NA's entfernen, da sonst der Datenverlust sehr gross wäre.
...
```{r}
plot_confusion_matrix(regression_all_predictions_threshold[[id_set]], val_sets[[id_set]]$has_card)
```
...
Der Random Forest ist ein wenig besser als der Decision Tree. Auch hier versuchen wir wieder mit RFE, das Modell zu reduzieren und somit vielleicht zu verbessern oder zumindest zu vereinfachen.
...
# Perform RFE
rfe_rf <- rfe(x = train %>% select(-has_card), y = train$has_card,
             sizes = c(seq(6, 20, 2)),
             rfeControl = control,
             method = "rf")
```
...
```{r}
print(paste("Die beste Anzahl von Parametern ist: ", length(predictors(rfe_rf))))
print("Die Parameter sind:")
print(predictors(rfe_rf))
```
...
for(i in 1:length(val_sets)) {
  ...
  random_forest_rfe <- randomForest(has_card ~ ., data = train_sets[[i]] %>% select(predictors(rfe_rf), has_card), method = "class")
...
for(i in 1:length(val_sets)) {
  ...
  random_forest_tuned <- randomForest(has_card ~ ., data = train_sets[[i]] %>% select(predictors(rfe_rf), has_card), method = "class", control = control)
...
Das XGBoost Modell wird mit der Library "xgboost" trainiert.
Da es Probleme mit nicht-numerischen Variabeln gab, wurden diese entfernt.
...
Bei der Hypterparameteroptimierung gehen wir wieder gleich vor wie beim Random Forest. Wir haben folgende Werte analysiert und ausgewertet:
...
for(i in 1:length(val_sets)) {
  ...
  xg_boost_tuned_metrics[[i]] <- get_metrics(xg_boost_tuned_predictions_threshold[[i]], val_sets[[i]]$has_card, "XGBoost")
...
Dabei betrachten wir pro Modellalgorithmus nur das jeweils beste bezüglich der Precision.
Also vergleichen wir vier Modelle: eine logistische Regression, ein Decision Tree, ein Random Forest und ein XGBoost.
...
plot_confusion_matrix(decision_tree_predictions_threshold[[id_set]], val_sets[[id_set]]$has_card, "Decision Tree")
plot_confusion_matrix(random_forest_predictions_threshold[[id_set]], val_sets[[id_set]]$has_card, "Random Forest")
plot_confusion_matrix(xg_boost_predictions_threshold[[id_set]], val_sets[[id_set]]$has_card, "XGBoost")
```
...
make_roc_plot_and_get_auc_list(list(regression_baseline_predictions[[id_set]], decision_tree_tuned_predictions[[id_set]], random_forest_tuned_predictions[[id_set]], xg_boost_predictions[[id_set]]), val_sets[[id_set]]$has_card)
...
imp_df <- data.frame(varImp(decision_tree))
imp_df <- imp_df %>% arrange(desc(Overall))
most_important_decision_tree <- rownames(imp_df)

imp <- xgb.importance(feature_names = colnames(train_preds), model = xg_boost)
...
Das beste Modell bezüglich der errechneten Scores, des RUC/AUC ist der Random Forest.
In der Top-N Analyse und der Confusion Matrix ist er mit der Beste und annähernd so performant wie XGBoost.
In Folge wird nun der Random Forest auf allen Trainingsdaten trainiert und auf den Testdaten analysiert.
