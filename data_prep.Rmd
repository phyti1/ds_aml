...
Die Transaktionen werden für Kreditkartenkäufer und Nicht-Käufer aggregiert, weshalb wir diese als Erstes anhand der Variable "has_card" trennen.
...
Für Nicht-Käufer ist kein Datum des Kreditkartenkaufs gegeben, da sie ja keine gekauft haben.
Es stellt sich also die Frage, von welchem Datum aus man das Rollup-Fenster erstellt.

Wir haben uns dazu entschieden, für jeden Käufer einen möglichst ähnlichen Nicht-Käufer zu finden.
Dafür werden Nichtkäufer mit gleichem Geschlecht und gleicher Region gesucht.
Das Kaufdatum des ähnlichen Käufers wird dann auf den Nicht-Käufer übertragen.
Ein weiteres Kriterium ist, dass der Nicht-Käufer mindestens eine Transaktion in dem Rollup-Fenster haben muss.
Jeder Nicht-Käufer wird maximal einmal als Match genommen, damit keine doppelten Kunden im Datensatz vorkommen.

Durch dieses Vorgehen benutzen wir nur einen Teil der Nicht-Käufer, haben aber direkte, ähnliche Vergleichspersonen, welche im gleichen Zeitraum keine Kreditkarten gekauft haben.
Ausserdem bringt das Vorgehen auch den Vorteil, dass die Daten für das Modell automatisch Balanced sind.
Dies macht das Trainieren des Modells einfacher, da es dazu beiträgt, dass das Modell nicht Biased gegenüber den Nicht-Käufern wird.
...
Wie hier sichtbar wird, konnte zu allen Kartenkäufer ein passender Nicht-Käufer gefunden werden.
...
Zum Schluss werden die Käufer und die Nicht-Käufer in einem Datensatz kombiniert.
...
Da das Kaufdatum des Nicht-Käufers auf das des Käufers gesetzt wird, sind die Verteilungen der beiden Klassen hier deckungsgleich.
...
Als nächstes wollen wir das Vermögen (balance) der Kunden anschauen.
Wir beobachten, ob sich das Vermögen von Kunden und nicht Kunden bezüglich Höhe und Entwicklung unterscheidet.
...
Kreditkartenkäufer haben im Schnitt ein höheres Vermögen als Nicht-Käufer.
Das Vermögen aller Kunden steigt tendenziell, wobei das der Käufer steiler als das der Nicht-Käufer steigt.
Ab dem siebten Monat scheint aber auch diese Vermögenssteigerung abzuflachen.

## Anzahl Transaktionen

Als nächstes wollen wir untersuchen, ob sich Kreditkartenkäufer und Nicht-Käufer durch die Anzahl getätigter Transaktionen unterscheiden.
...
Kreditkartenkäufer haben also im Schnitt mehr Transaktionen getätigt als Nicht-Käufer im Rollup-Fenster.
...
Beide Klassen haben mehr positive Transaktionen als negative, wobei Käufer bei positiven sowie bei negativen durchschnittlich mehr haben.
Bei den Einnahmen ist der Unterschied ein wenig höher als bei den Ausgaben.
...
Zu diesem Zweck definieren wir einige Funktionen, damit wir weniger redundanten Code haben und unsere Ergebnisse in einem einheitliche, vergleichbaren Format daherkommen.
...
Accuracy (Genauigkeit): Die Accuracy ist der Prozentsatz der Vorhersagen, die mit den tatsächlichen Werten übereinstimmen.
Sie wird berechnet als Anzahl der korrekten Vorhersagen geteilt durch die Gesamtzahl der Vorhersagen.

Cohen's Kappa (Cohen's Kappa): Cohen's Kappa ist eine Messgröße für die Qualität von binären Klassifikationen.
...
Matthews correlation coefficient (Matthews Korrelation): Der Matthews Korrelation Coefficient (MCC) ist eine Messgröße für die Qualität von binären Klassifikationen.
...
Precision (Präzision): Die Präzision ist der Prozentsatz der Vorhersagen, die tatsächlich korrekt waren, unter der Annahme, dass alle Vorhersagen korrekt sind.
...
Recall (Erinnerung): Der Recall ist der Prozentsatz der tatsächlich positiven Werte, die korrekt vorhergesagt wurden.
...
F1 score (F1-Wert): Der F1-Wert ist ein Maß für die Qualität von binären Klassifikationen, das die Harmoniesche Mischung von Präzision und Recall darstellt.
...
Sie zeigt die Leistung des Klassifikators bei verschiedenen Schwellenwerten an, die zur Unterscheidung zwischen zwei Klassen verwendet werden.
...
Da die logistische Regression eine Zahl zwischen 0 und 1 zurückgibt, müssen wir anhand eines Thresholds die Werte zu TRUE und FALSE umwandeln.
...
Als erstes wollen wir uns die durchschnittlichen Metriken anschauen.
...
Das Baseline Modell ist mit einer Precision von ca. 0.78 und einem F1 Score von 0.78 schon einmal nicht schlecht. Nun soll dieses Baseline-Modell verbessert werden.

### Regressionsmodell mit allen Daten

Als erstes probieren wir, das gleiche Modell (Logistische Regression) mit mehr Input-Parametern zu trainieren. Da die logistische Regression Probleme mit Faktoren hat, welche nur im Trainings- bzw.
...
Es gibt 18 Kunden, welche fehlende Werte haben. Da unser Datensatz mit nur etwa 1200 Kunden schon klein ist, wollen wir diese nicht auch noch entfernen. Daher imputieren wir die numerischen fehlenden Werte mit dem Median und die fehlenden kategorialen Werte mit dem Wert, welcher am meisten vorkommt.
...
Die logistische Regression mit allen Daten konnte die Scores leider nicht verbessern. Die Precision ist auf einem sehr ähnlichen Wert wie bei unserem Baseline Modell.


### Decision Tree

Als erstes machen wir einen Decision Tree. Für den Decison Tree verwenden wir das Package "rpart".
...
Der Decision Tree konnte alle Metriken steigern. Nun wollen wir probieren, diesen noch ein wenig zu verbessern.

Decision Trees sind in der Lage, mit einer grossen Anzahl von Merkmalen umzugehen. Es kann aber auch von Vorteil sein, nicht alle Merkmale zu benutzen, um Überanpassung zu vermeiden und dadurch die Performance des Modells zu verbessern. Dies kann auch die Interpretierbarkeit des Modells erhöhen und die Kosten für das Training und die Auswertung des Modells reduzieren. Daher wollen wir als nächstes untersuchen, ob das Modell besser wird, wenn nicht alle Features verwendet werden.
...
Da der RFE bereits mit Cross-Validation arbeitet, benutzen wir hier das gesamte Trainingsset. Danach machen wir nochmals unsere eigene Kreuzvalidierung mit dem gefundenen Subset der Input-Parametern. Wir suchen die optimalen Parameter für die Anzahlen 1 bis 20. Dabei optimiert das Modell auf die Metrik Accuracy, Precision ist leider nicht möglich. Die Parameter der besten Anzahl werden wir anschliessend mit unserer eigenen Kreuzvalidierung auswerten und mit dem vorherigen Modell vergleichen
...
Mit den reduzierten Features konnte der Decision Tree die Precision ein wenig erhöhen. Das ist grossartig, da der Decision Tree jetzt nicht nur simpler, sondern sogar besser ist.

### Random Forest

Nun wollen wir eine Erweiterung des Decisions Tree anschauen: Der Random Forest. Random Forest ist ein Ensemble von Decision Trees und kann komplexe Daten besser handhaben und das Überanpassungsproblem reduzieren.
...
Der Random Forest schneidet ein wenig schlechter ab, als der Decision Tree mit den reduzierten Features. Wir versuchen auch hier, die Features zu reduzieren. Dafür brauchen wir wieder die evaluierten Features von der RFE beim Decision Tree.
...
Um den Random Forest noch weiter zu verbessern, wollen wir die verschiedenen Hyperparameter optimieren. Hyperparameteroptimierung (auch Tuning genannt) ist der Prozess, die beste Set von Hyperparametern für ein maschinelles Lernmodell zu finden. Da wir Probleme mit dem GridSearch haben, haben wir folgende Werte für die definierten Parameter probiert und den jeweils besten anhand der Precision ausgewertet:

mtry : die Anzahl der Variablen, die zufällig als Kandidaten an einer Trennung ausgewählt werden.
Probierte Werte: 15, 70, 110
Bester Wert: 15

splitrule : die Trennregel, die verwendet wird, um die Qualität einer Trennung zu bewerten.
Probierte Werte: "gini", "extratrees", "variance"
Bester Wert: "extratrees"

ntree : die Anzahl der Bäume, die im RandomForest gewachsen sind.
Probierte Werte: 100, 500, 1000
Bester Wert: 500

max.depth : die maximale Tiefe der Bäume
Probierte Werte: 10, 15, 20
Bester Wert: 10
...
Eine weitere Erweiterung des Decision Trees ist XGBoost. Das XGBoost Modell wird mit der Library "xgboost" trainiert. Da es Probleme mit nicht-numerischen Variabeln gab, wurden diese für dieses Modell entfernt.
...
Die Scores sind fast identisch zu denen des Random Forests. Wir probieren hier auch, die Hyperparameter anzupassen. Bei der Hypterparameteroptimierung gehen wir wieder gleich vor wie beim Random Forest. Wir haben folgende Werte analysiert und ausgewertet:

"nrounds" gibt die Anzahl der Iterationen an, die der Algorithmus durchführen soll. Je höher die Anzahl, desto länger dauert das Training, aber es besteht auch die Möglichkeit, dass die Genauigkeit des Modells steigt. Eine hohe Anzahl Iterationen kann aber auch zu Überanpassung führen
Probierte Werte: 1000, 2000, 5000
Bester Wert: 1000

"eta" gibt den Schrittweiten-Faktor an, mit dem die Gewichtungen des Modells bei jeder Iteration aktualisiert werden. Ein kleinerer Wert führt zu kleineren Schritten und damit zu einer größeren Anzahl von Iterationen, um die optimale Lösung zu erreichen.
Probierte Werte: 0.1, 0.2, 0.3
Bester Wert: 0.1

"max_depth" gibt die maximale Tiefe des Baumes an. Ein höherer Wert ermöglicht es dem Modell, komplexere Entscheidungen zu treffen, aber es kann auch zu Overfitting führen.
Probierte Werte: 3, 5, 10
Bester Wert: 3

"gamma" gibt den Mindestverlustreduktionswert an, der erforderlich ist, um einen Knoten weiter zu teilen. Ein höherer Wert führt dazu, dass weniger Knoten weiter geteilt werden und damit ein simpler Baum erstellt wird.
Probierte Werte: 0, 0.1, 0.2, 1
Bester Wert: 0

"min_child_weight" gibt das Minimum an Gewicht (bezogen auf die Anzahl der Beobachtungen) an, das ein Knoten haben muss, um weiter geteilt zu werden. Ein höherer Wert führt dazu, dass weniger Knoten weiter geteilt werden und damit ein simpler Baum erstellt wird.
Probierte Werte: 1, 3, 5
Bester Wert: 1
...
Um nun herauszufinden, welches unserer Modelle das beste ist, vergleichen wir die Modelle auf verschiedene Arten und Weisen.
Dabei betrachten wir pro Modellalgorithmus nur jeweils ein Modell: 
Bei der Logistischen Regression nehemn wir die "Logistic Regression All", beim Decision Tree "Decision Tree (RFE)", beim Random Forest "Random Forest (Tuned)" und beim XGBoost "XGBoost (Tuned)"
...
Das beste Modell bezüglich Train CV-Score ist der Random Forest. Er ist jedoch bei allen Kennzahlen fast deckungsgleich wie der XGBoost. 
Dies macht Sinn, da XGBoost eine Erweiterung des Random Forests ist. 
...
In dieser Darstellung werden die Verteilungen der CV-Scores pro Modell aufgezeigt.
Ein gut optimiertes Modell sollte auf den Cross-Validation Sets möglichst ähnliche Scores aufweisen.
Damit kann aufgezeigt werden, dass das Modell auf die verschiedenen Testdaten nicht overfittet.
Wie wir hier sehen sind die Variationen der Precision eher klein. Die ähnlichsten Precision Resultate hat der Decision Tree.


## Vergleich der Top-N-Listen

Hier vergleichen wir die Top-N-Listen der Modelle.
Für jedes Modell wird eine Top-20 Liste der Kunden vom ersten und letzten Validierungsset der Kreuzvalidierung mit den höchsten Werten bei der Prediction gemacht.
All diese Kunden müssten demnach Käufer sein.
In den beiden Tabellen stellen wir dar, ob die Modelle auf den jeweiligen Positionen recht behalten haben oder nicht.
...
In dieser Darstellungen sind die Anzahl Fehlklassifikationen auf alle 10 Cross Validation Datensätze aufsummiert. Das perfekte Modell hätte auf allen Werten eine 0, also auf allen Top-N Vorhersagen keine Fehler gemacht.
...
Um dies auf den Trainingsdaten tun zu können, wurde ein zufälliges Train/Val Packet gewählt.
...
Als nächstes wollen wir die wichtigsten Features für jedes Modell anschauen. Das Paket DALEX hat leider auf unseren Modellen nicht richtig funktioniert, weshalb wir die Wichtigkeit der Features bei jedem Modell anders auslesen müssen. 

Beim Decision Tree und dem Random Forest wird die Wichtigkeit nach der Metrik "Overall variable Importance" ausgelesen. Die Overall variable importance bezieht sich auf den Durchschnitt der Verringerung der Unreinheit (z.B. Gini-Impurities), die durch Aufteilungen anhand einer bestimmten Variable in allen Bäumen im Wald verursacht wird. Es ist ein Maß dafür, wie sehr eine Variable zur Genauigkeit des Modells beiträgt. Eine Variable mit einem hohen Gesamtwert der Wichtigkeit wird wahrscheinlich ein guter Prädiktor für die Zielvariable sein. Mit Hilfe des Wichtigkeitswerts kann man die wichtigsten Merkmale identifizieren, die im Modell verwendet werden.

Für den XGBoost wird die Metrik "Cover" verwendet. "Cover" gibt an, wie oft ein Merkmal im Baumaufteilungen verwendet wird. Es ist ein Maß dafür, wie häufig ein Merkmal von dem Modell ausgewählt wird, um Entscheidungen zu treffen
...
Obwohl wir die Werte der Metriken nicht vergleichen können, so können wir die Reihenfolge der Wichtigkeit der Variablen in den verschiedenen Modellen anschauen. Dafür wird eine Tabelle erstellt, welche bei allen Modellen die wichtigsten 10 Variablen anzeigt.
...
In Folge wird nun versucht, ob der Random Forest noch weiter reduziert werden könnte, ohne dass er grosse Performance-Verluste hat. Anschliessend trainieren wir ihn auf allen Trainingsdaten und analysieren ihn auf den Testdaten.

## Reduzieren des besten Modelles

```{r}
length(most_important_random_forest)
```
...
for (i in 1:length(most_important_random_forest)) {
...
for(j in 1:length(val_sets)) {
  ...
  train_sets[[j]]$has_card <- as.factor(train_sets[[j]]$has_card)
  
  final_model_reduced <- randomForest(has_card ~ ., data = train_sets[[j]] %>% select(all_of(most_important_random_forest), has_card), method = "class", control = control)
  ...
  final_model_reduced_metrics[[j]] <- get_metrics(final_model_reduced_predictions_threshold[[j]], val_sets[[j]]$has_card, "Random Forest (Tuned)")
...
print(paste("Best precisions:", precisions[best_index]))
print(paste("Number of variables:", best_index))

# Plot the f1-scores
plot(1:length(precisions), precisions, type = "l", xlab = "Anzahl Variabeln", ylab = "Precision")
...
random_forest_refit <- randomForest(has_card ~ ., data = train %>% select(most_important_random_forest[1:best_index], has_card), method = "class", mtry = 100)
...
random_forest_refit_metrics <- get_metrics(random_forest_refit_predictions_threshold, test$has_card, "Random Forest Refit Off Time")
random_forest_refit_metrics
```

Die Scores auf dem Testdaten fallen wie zu erwarten war etwas tiefer aus als die gemittelten Train Scores.
Die Accuracy bleibt jedoch noch über 0.8.
...
random_forest_refit_metrics <- get_metrics(random_forest_refit_predictions_threshold, test$has_card, "Random Forest Refit Random")
random_forest_refit_metrics
```
...
Bei unserem Model handelt es sich um ein Product Affinity Model.
Es soll voraussagen, ob eine Person (hier eine Kund:in der Bank) ein bestimmtes Produkt kauft (hier eine Kreditkarte) oder nicht.
Ausserdem gibt unser Modell auch an, mit welcher Wahrscheinlichkeit sie diese kauft.
Dies könnte von grossem Mehrwert für eine Bank sein, da sie anhand des Modells die Marketing-Strategie optimieren und ihre Ressourcen gezielter einsetzen können.

Es kann Banken helfen, ihre Marketing-Bemühungen besser zu segmentieren, indem es zeigt, wer wahrscheinlich an einer Kreditkarte interessiert ist und wer nicht.
Dies kann dazu beitragen, die Conversion-Rate zu verbessern und die Kosten für Marketing-Aktionen zu senken.
...
Wenn das Modell vorhersagt, wer wahrscheinlich eine Kreditkarte kaufen wird, können Banken ihren Kunden personalisierte Angebote und Services anbieten, die auf ihre individuellen Bedürfnisse und Vorlieben abgestimmt sind.
...
mit einem hohen Rückzahlungsprozentsatz für bestimmte Kategorien von Einkäufen oder mit Reiseversicherungen und anderen Vorteilen, die für den Kunden besonders wichtig sind.
