...
Auch diese Auswertung zeigt, die Hyperparameteroptimierung hat kaum Verbesserungen gegenüber dem unoptimierten Modell gezeigt. 
...
Das beste Modell bezüglich Train CV-Score ist der Random Forest.
Er ist jedoch bei allen Kennzahlen fast deckungsgleich wie der XGBoost.
...
Wie wir hier sehen sind die Variationen der Precision eher klein.
Die ähnlichsten Precision Resultate hat der Decision Tree.
...
In der nachfolgenden Tabelle stellen wir dar, wie viele Male das jeweilige Modell auf der jeweiligen Position recht behalten hat oder nicht.
...
Betrachten wir nur die Falschklassifikationen der Top-3 Vorhersagen, sehen wir, dass "Random Forest" und "XGBoost" am besten abschneiden.
Der "Decision Tree" und die "logistische Regression" haben auf die Top-3 Vorhersagen drei oder mehr falsche Vorhersagen gemacht, die Forest Modelle nur null bis einen.
XGBoost schneidet hier insgesamt am Besten ab.
...
Durch den AUC wird deutlich, dass der Random Forest hier leicht die Nase vorn hat.
...
Für die erste Analyse trennen wir train/test zeitlich. Damit sind die makroökonomischen 
...
Bis jetzt haben wir alle Modelle mit Off-Time-Daten evaluiert.
Zum Schluss möchen wir noch schauen, ob unsere Metriken bei zufälligen Testdaten besser werden würden.
Dafür müssen wir einen neuen Train-Test-Split erstellen.
