...
Zudem wird das Alter der clients im Jahre 2022 herausextrahiert.
...
client <- client %>% mutate(age = 2022 - year(birth_number))
```
...
```{r}
client_analytical_record <- full %>% filter(!is.na(date))
sum(duplicated(client_analytical_record$client_id))
```
