...
# nötige Packete
packages <- c("tidyverse", "data.table", "tidymodels", "lubridate", "caret", "pROC", "randomForest", "rpart", "rpart.plot", "xgboost")
...
print("Finde passende non-buyers")
...
get_metrics_mean <- function(pred_list, true_list)
{
  # berechne das Mittel aller Scores über alle folds
  if(class(true_list[[1]]) == "data.frame")
  {
    true_list <- lapply(val_sets, function(x) x$has_card)
  }
  metrics <- list()
  for(i in 1:length(pred_list))
  {
    metrics[[i]] <- get_metrics(pred_list[[i]], true_list[[i]])
  }
  metrics_mean <- colMeans(bind_rows(metrics))
  return(metrics_mean)
}
...
```{r}
train_glm <- function(train_df, val_df)
{
  model <- glm(has_card ~ ., data = train_df, family = binomial)
  # Make predictions on the validation data
  predictions <- predict(model, val_df, type = "response")

  threshold <- 0.5
  predictions_threshold <- ifelse(predictions > threshold, TRUE, FALSE)
  return(list(predictions_threshold, model, predictions))
}

# requires: train_sets, val_sets, train, test datasets
get_cv_scores <- function(train_routine, train_sets, val_sets, train, test)
{
  predictions_list <- c()
  # Fit the model on the training data
  for(i in 1:length(val_sets))
  {
    predictions_threshold <- train_routine(train_sets[[i]], val_sets[[i]])[[1]]
    predictions_list[[i]] <- predictions_threshold
  }
  # refit on full training dataset
  refit_result <- train_routine(train, test)
  predictions_threshold <- refit_result[[1]]
  model <- refit_result[[2]]
  
  return(list(get_metrics_mean(predictions_list, val_sets), predictions_threshold, model))
}

cv_return <- get_cv_scores(train_glm, train_sets, val_sets, train, test)
metrics <- cv_return[[1]]
predictions_threshold <- cv_return[[2]]
model <- cv_return[[3]]
...
```{r}
suppressWarnings({
  predictions_train <- predict(model, train, type = "response")
...
```{r}
suppressWarnings({
  cv_results <- get_cv_scores(train_glm, train_sets, val_sets, train, test)
  metrics <- cv_return[[1]]
  predictions_threshold <- cv_return[[2]]
  model <- cv_return[[3]]
  metrics
...
```{r}
predictions_train <- train_glm(train, train)[[3]]
best_threshold <- find_best_threshold(predictions_train, train$has_card)
```
...
```{r}
predictions_threshold <- train_glm(train %>% select(max_difference_12, max_difference_11, has_card), test)[[1]]
...
```{r}
train_dt <- function(train_df, val_df)
{
  model <- rpart(has_card ~ ., data = train_df, method = "class")

  # Make predictions on the test data (second column is for probability of true)
  predictions <- predict(model, val_df, type = "prob")[, 2]
...
cv_return <- get_cv_scores(train_dt, train_sets, val_sets, train, test)
metrics <- cv_return[[1]]
predictions_threshold <- cv_return[[2]]
model <- cv_return[[3]]
...
plot_confusion_matrix(predictions_threshold, test$has_card)
make_roc_plot_and_get_auc(predictions_threshold, test$has_card)
predictions_train <- predict(model, train, type = "prob")[, 2]
best_threshold <- find_best_threshold(predictions_train, train$has_card)
rpart.plot(model)
```
...
```{r}
final_df_rf <- final_df_simplified
final_df_rf$has_card <- as.factor(final_df_rf$has_card)

splits <- split_data(final_df_rf, test_size = 0.2)
train_sets <- splits$train_sets
val_sets <- splits$val_sets
train <- splits$train
test <- splits$test

train_rf <- function(train_df, val_df)
{
  # Fit a random forest model to the training data
  model <- randomForest(has_card ~ ., data = train_df, method = "class")
...
cv_return <- get_cv_scores(train_rf, train_sets, val_sets, train, test)
metrics <- cv_return[[1]]
predictions_threshold <- cv_return[[2]]
model <- cv_return[[3]]
metrics
...
set.seed(27)
# Fit a random forest model to the training data
model <- randomForest(has_card ~ ., data = train %>% select(has_card, importance_values$variable[1:best_index]), method = "class")
...
final_df_xgb <- final_df_simplified

final_df_xgb$has_card <- as.numeric(final_df_xgb$has_card)

splits <- split_data(final_df_xgb, test_size = 0.2)
train_sets <- splits$train_sets
val_sets <- splits$val_sets
train <- splits$train
test <- splits$test
...
train_xgb <- function(train_df, val_df)
{
  # Convert the data to a matrix
  mat_train <- as.matrix(train_df %>% select(has_card, importance_values$variable[1:7]))
  mat_val <- as.matrix(val_df %>% select(has_card, importance_values$variable[1:7]))
  
  # Split the labels and predictors
  train_labels <- mat_train[, 1]
  train_preds <- mat_train[, -1]
  test_labels <- mat_val[, 1]
  test_preds <- mat_val[, -1]
  
  param <- list(max_depth = 100, eta = 0.1, nthread = 2)
    
  # Train the model
  model <- xgboost(data = train_preds, label = train_labels, nrounds = 1000, objective = "binary:logistic", verbose = 0, maximize = "F1", params = param)

  # Make predictions on the test data
  predictions <- predict(model, test_preds)
...
# train, Evaluate the model's performance
cv_results <- get_cv_scores(train_xgb, train_sets, val_sets, train, test)
metrics <- cv_return[[1]]
predictions_threshold <- cv_return[[2]]
model <- cv_return[[3]]
metrics
...
model <- train(x = train_preds, y = train_labels, method = "xgbTree", trControl = trainControl(method = "cv", number = 5), tuneGrid = param_grid, metric = "F1", verbose = FALSE, verbosity = 0)
...
# Evaluate the model's performance
get_metrics(predictions_threshold, test_labels)
```
