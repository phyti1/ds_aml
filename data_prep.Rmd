...
In Folge wird die Nummer in ihre Datumsräpresentation konvertiert und die Spalte "gender" als male/female aufgeschlüsselt. Zudem wird das Alter der Kunden im Jahr 1999 berechnet, da der Datensatz aus diesem Jahr stammt.
...
In den Transaktionen muss das Datum gemäss Format YYMMDD konvertiert und die verschiedenen tschechischen Ausdrücke übersetzt werden. Ein weiteres Problem ist, dass Amount immer eine positive Zahl ist, auch wenn sich die Balance negativ verändert. Dies soll später behandelt werden.
...
Es gibt also keine Kunden, welche mehrere Darlehen bezogen haben. Die Loans sollen an das gesamte Dataframe mittels Left Join agehängt werden. So gibt es Kunden, welche keine Loans haben. Bei diesen sollen die fehlenden numerischen Werte durch 0 ersetzt und der Status auf "no_loan" geändert werden. 
...
Nun müssen nur noch die Transaktionsdaten zum Dataframe hinzugefügt werden. Diese müssen aber anders zusammengefasst werden.
...
Bei den Transaktionen ist jeweils die neue Balance und der Betrag der Transaktion angegeben. Das Problem dabei ist, dass alle Beträge positiv sind, auch wenn sie eigentlich abgezogen werden. Das Vorzeichen der Transaktion kann jedoch der Spalte "type" entnommen werden.
...
Es gibt zwei Transaktionstypen: credit und withdrawl, welche das Vorzeichen der Transaktion definieren. Daraus wird in Folge die Differenz und das prev_balance berechnet.
...
df <- df %>% 
  summarize(transactions = {
    ...
    difference <- ifelse(type == "credit", amount, amount * -1)
...
```{r}
transactions %>% filter(sum(abs(balance - prev_balance)) != sum(abs(difference)))
```

Wie wir in diesem Test sehen, entspricht die Summe der Transaktionsdifferenzen der Differenz des Kontostandes. 
...
Es existieren oft Monate, bei denen ein Client keine Transaktionen gemacht hat. In diesen Monaten werden in Folge jeweils eine leere Transaktion eingefügt, welche die Balance der letzten Transaktion des Vormonats trägt. Für die non-buyers muss dies ebenfalls durchgeführt werden, weshalb dies als Funktion implementiert wird.
...
get_summary_df <- function(filtered_df)
{
  ...
  group_by(account_id, issued, month) %>%
  summarise(
...
Nach der Imputation gibt es keine Kunden mehr, welche nicht 12 Monate mit Transaktionen beinhalten. 
...
summary_df_buyers <- merge(summary_df_wide, subset(card_buyers, select = -issued), by = "account_id")
```
...
Dafür werden zuerst Nichtkäufer gleichen Geschlechts und Region gesucht.
Diese sollen im Zeitfenster issued-13 bis issued-1 Monate vor Kartenkauf des korrespondierenden Käufers mindestens eine Transaktion haben.
Am Ende wird der Nichtkäufer mit dem ähnlichsten Alter gewählt.
...
for (i in 1:nrow(card_buyers)) 
{
  ...
  similar_non_buyers_temp <- non_buyers %>% filter(gender == current_buyer$gender & 
                                                   ...
                                                   #abs(age - current_buyer$age) <= 100)
  
  # damit nicht der gleiche non_buyer doppelt verwendet wird
  similar_non_buyers_temp <- similar_non_buyers_temp[!similar_non_buyers_temp$client_id %in% similar_non_buyers$client_id,]
  ...
  #similar_non_buyers_temp_copy <- similar_non_buyers_temp %>% filter(region == current_buyer$region)
  #if(nrow(similar_non_buyers_temp_copy) > 0)
  #{
  #  similar_non_buyers_temp <- similar_non_buyers_temp_copy
  #}
  #else
  #{
  #  print(paste(current_buyer$account_id, "could not filter region"))
  #}
  # age difference will be filtered later
  #abs(age - current_buyer$age) <= 5,
           
  
  if(nrow(similar_non_buyers_temp) > 0)
  {
    ## (ungebraucht) Implementierung zur Herausfilterung von Monaten ohne Transaktionen
    # filtere alle non-boyers, welche nicht genug Transaktionen haben
    # finde alle Transaktionen der selektierten non-buyers
    # non_buyer_transactions <- merge(transactions, similar_non_buyers_temp, by='account_id')
    # filtere diese Transaktionen auf issued-13m bis issued-1m
    #non_buyer_transactions <- non_buyer_transactions %>% filter(
    #  date.x >= floor_date(current_buyer$issued %m-% months(13), 'month') &
    #  date.x <= floor_date(current_buyer$issued %m-% months(1), 'month') - days(1))
    
    # finde Anzahl Monate mit Transaktionen
    #non_buyer_transactions$month <- format(non_buyer_transactions$date.x, "%Y-%m")
    #month_counts <- non_buyer_transactions %>% group_by(account_id) %>% summarise(month_count = n_distinct(month))
  
    # filtere alle accounts, welche nicht alle 12 Monate hat
    #month_counts <- month_counts %>% filter(month_count == 12)
    #similar_non_buyers_temp <- similar_non_buyers_temp %>% filter(account_id %in% month_counts$account_id)
...
print(paste(length(unique(similar_non_buyers$account_id)), length(unique(card_buyers$account_id))))
```


Wie hier sichtbar wird, kann zu allen Kartenkäufer einen passenden Nichtkäufer gefunden werden. Dafür musste jedoch die Altersdifferenz-Einschränkung aufgehoben werden. Es entsteht ein balanced Dataset für die Weiterentwicklung.
...
```{r}
# find missings
non_buyer_transactions <- merge(transactions, similar_non_buyers[, c("account_id", "client_id", "issued")], by='account_id')
filtered_df <- filter_time_range(non_buyer_transactions)
print(length(unique(filtered_df$account_id)))
unique(non_buyer_transactions[!non_buyer_transactions$account_id %in% unique(filtered_df$account_id),]$account_id)
```

Wie diese Analyse zeigt, existieren keine non-buyers ohne Transaktionen im definierten Zeitfenster mehr.
...
```{r}
print(paste(nrow(summary_df_non_buyers), nrow(summary_df_buyers)))
```

Die Anzahl Zeilen in den summary_df's sind identisch. 
...
Auch hier haben wieder keine Kunden weniger als 12 kontinuierliche Monate mit Transaktionen, da diese durch die Imputation gefüllt wurden.
...
summary_df_non_buyers <- merge(summary_df_wide, subset(similar_non_buyers, select = -issued), by = "account_id")
print(nrow(summary_df_non_buyers))
```

Wir haben nun wieder die 706 Zeilen auch für die non-buyers.
...
Jetzt müssen noch einige Variabeln entfernt werden.
...
```{r}
colSums(is.na(final_df))[colSums(is.na(final_df)) > 0]
```

Bei den standard deviations exisiteren nun viele NA's, da bei den imputierten Monaten nur eine Transaktion pro Monat existiert und dadurch durch n-1 = 0 dividiert wird. Diese NA's werden mit 0 imputiert.
...
ggplot() +
  ...
  labs(x = "Month", y = "Average Mean Balance") +
...
# Durchschnittliche Werte für has_card == TRUE berechnen
averages_true <- colMeans(final_df[final_df$has_card == TRUE,
                                   c("mean_difference_1", "mean_difference_2", "mean_difference_3",
                                     "mean_difference_4", "mean_difference_5", "mean_difference_6",
                                     "mean_difference_7", "mean_difference_8", "mean_difference_9",
                                     "mean_difference_10", "mean_difference_11", "mean_difference_12")])
...
ggplot() +
  geom_line(aes(x = 1:12, y = averages_true), color = "steelblue") +
  geom_line(aes(x = 1:12, y = averages_false), color = "red") +
  labs(x = "Month", y = "Average Mean Difference") +
  scale_color_manual(name = "", values = c("steelblue" = "Buyers", "red" = "Non Buyers")) +
  scale_x_continuous(limits = c(1, 12))
```
...
ggplot(final_df, aes(x = issued, fill = factor(has_card))) +
  ...
  labs(x = "Issued Date", y = "Density", fill = "Has Card") +
  scale_x_date(limits = range(final_df$issued))
```

```{r}
# Calculate the average sum of count_negative_difference for has_card == TRUE
avg_sum_true <- mean(rowSums(final_df[final_df$has_card == TRUE, grep("count_negative_difference", colnames(final_df))]))

# Calculate the average sum of count_negative_difference for has_card == FALSE
avg_sum_false <- mean(rowSums(final_df[final_df$has_card == FALSE, grep("count_negative_difference", colnames(final_df))]))

# Create the bar plot
ggplot() +
  geom_bar(aes(x = c("TRUE", "FALSE"), y = c(avg_sum_true, avg_sum_false)), stat = "identity", fill = "steelblue") +
  labs(x = "Has Card", y = "Average Sum of Count Negative Difference")
```

```{r}
# Calculate the average sum of count_negative_difference for has_card == TRUE
avg_sum_true <- mean(rowSums(final_df[final_df$has_card == TRUE, grep("count_positive_difference", colnames(final_df))]))

# Calculate the average sum of count_negative_difference for has_card == FALSE
avg_sum_false <- mean(rowSums(final_df[final_df$has_card == FALSE, grep("count_positive_difference", colnames(final_df))]))

# Create the bar plot
ggplot() +
  geom_bar(aes(x = c("TRUE", "FALSE"), y = c(avg_sum_true, avg_sum_false)), stat = "identity", fill = "steelblue") +
  labs(x = "Has Card", y = "Average Sum of Count Positive Difference")
```

```{r}
card_buyers_with_loan <- final_df[final_df$has_card & !is.na(final_df$amount),]
card_buyers_without_loan <- final_df[final_df$has_card & is.na(final_df$amount),]
non_buyers_with_loan <- final_df[!final_df$has_card & !is.na(final_df$amount),]
non_buyers_without_loan <- final_df[!final_df$has_card & is.na(final_df$amount),]

# Calculate the number of people in each group
card_buyers_with_loan_count <- length(card_buyers_with_loan$has_card)
card_buyers_without_loan_count <- length(card_buyers_without_loan$has_card)
non_buyers_with_loan_count <- length(non_buyers_with_loan$has_card)
non_buyers_without_loan_count <- length(non_buyers_without_loan$has_card)

# Create the bar plot
ggplot() +
  geom_bar(aes(x = c("Card buyers with loan", "Card buyers without loan", "Non-buyers with loan", "Non-buyers without loan"), y = c(card_buyers_with_loan_count, card_buyers_without_loan_count, non_buyers_with_loan_count, non_buyers_without_loan_count)), stat = "identity") +
  labs(x = "Group", y = "Number of People")
```


```{r}
# Create the bar plot
ggplot(final_df, aes(x = region, fill = factor(has_card))) +
  geom_bar(position = "dodge") +
  labs(x = "Region", y = "Number of People", fill = "Has Card")
```
...
Das Regressionsmodell wird auf den Trainingsdaten trainiert. Wir verwenden die Funktion glm von der Library "stats".

```{r}
library(DALEX)
# Fit the model on the training data
regression_baseline <- glm(has_card ~ ., data = train, family = binomial)

cv_method <- trainControl(method = "cv", number = 10)


#regression_baseline <- train(has_card ~ ., data = train, method = "glm", trControl = cv_method, family = "binomial")
...
regression_baseline_predictions <- predict(regression_baseline, test, type = "response")

threshold <- 0.5
regression_baseline_predictions_threshold <- ifelse(regression_baseline_predictions > threshold, TRUE, FALSE)


regression_baseline_metrics <- get_metrics(regression_baseline_predictions_threshold, test$has_card, "Logistic Regression Baseline")
...
```{r}
make_roc_plot_and_get_auc(regression_baseline_predictions_threshold, test$has_card)
```

```{r}
plot_feature_importance(regression_baseline)
```
...
Der F1 ist eine Zusammenfassung der Precision und der Recall. Wir untersuchen, mit welchem Threshold der höchste F1-Score erzielt werden kann. Wir probieren den Threshold auf den Trainingsdaten zu optmieren.

```{r}
suppressWarnings({
  predictions_train <- predict(regression_baseline, train, type = "response")
...
```{r}
regression_baseline_predictions_best_thresholds <- ifelse(regression_baseline_predictions > best_threshold, TRUE, FALSE)
regression_baseline_best_threshold_metrics <- get_metrics(regression_baseline_predictions_best_thresholds, test$has_card, "Logistic Regression Baseline (Best Threshold)")

metrics_all <- bind_rows(metrics_all, regression_baseline_best_threshold_metrics)
regression_baseline_best_threshold_metrics
```
...
```{r}
# Fit the model on the training data
regression_all <- glm(has_card ~ ., data = train, family = binomial)

cv_method <- trainControl(method = "cv", number = 10)
#regression_all <- train(has_card ~ ., data = train, method = regression_all, trControl = cv_method)
...
```{r}
# Make predictions on the test data
regression_all_predictions <- predict(regression_all, test, type = "response")

threshold <- 0.5
regression_all_predictions_threshold <- ifelse(regression_all_predictions > threshold, TRUE, FALSE)

regression_all_metrics <- get_metrics(regression_all_predictions_threshold, test$has_card, "Logistic Regression (All Data)")
metrics_all <- bind_rows(metrics_all, regression_all_metrics)
regression_all_metrics
```
...
```{r}
make_roc_plot_and_get_auc(regression_all_predictions_threshold, test$has_card)
```
...
```{r}
predictions_train <- predict(regression_all, train, type = "response")
best_threshold <- find_best_threshold(predictions_train, train$has_card)
```

```{r}
regression_all_predictions_best_thresholds <- ifelse(regression_all_predictions > best_threshold, TRUE, FALSE)
regression_all_best_threshold_metrics <- get_metrics(regression_all_predictions_best_thresholds, test$has_card, "Logistic Regression All (Best Threshold)")

metrics_all <- bind_rows(metrics_all, regression_all_best_threshold_metrics)
regression_all_best_threshold_metrics
```
...
```{r}
regression_two <- glm(has_card ~ ., data = train %>% select(max_difference_12, max_difference_11, has_card), family = binomial)


cv_method <- trainControl(method = "cv", number = 10)
#regression_two <- train(has_card ~ ., data = train, method = regression_two, trControl = cv_method)

regression_two_explainer <- explain(regression_two, data = train)

regression_two_predictions <- predict(regression_two, test, type = "response")

threshold <- 0.5
regression_two_predictions_threshold <- ifelse(regression_two_predictions > threshold, TRUE, FALSE)

regression_two_metrics <- get_metrics(regression_two_predictions_threshold, test$has_card, "Logistic Regression (Two Variables)")
metrics_all <- bind_rows(metrics_all, regression_two_metrics)
regression_two_metrics
```
...
decision_tree <- rpart(has_card ~ ., data = train, method = "class")

cv_method <- trainControl(method = "cv", number = 10)
#decision_tree <- train(has_card ~ ., data = train, method = decision_tree, trControl = cv_method)

decision_tree_explainer <- explain(decision_tree, data = train)

# Make predictions on the test data (second column is for probability of true)
decision_tree_predictions <- predict(decision_tree, test, type = "prob")[, 2]

threshold <- 0.5
decision_tree_predictions_threshold <- ifelse(decision_tree_predictions > threshold, TRUE, FALSE)

decision_tree_metrics <- get_metrics(decision_tree_predictions_threshold, test$has_card, "Decision Tree")
...
make_roc_plot_and_get_auc(decision_tree_predictions_threshold, test$has_card)
...
random_forest <- randomForest(has_card ~ ., data = train, method = "class")

cv_method <- trainControl(method = "cv", number = 10)
#random_forest <- train(has_card ~ ., data = train, method = random_forest, trControl = cv_method)

random_forest_explainer <- explain(random_forest, data = train)

# Make predictions on the test data (second column is for probability of true)
random_forest_predictions <- predict(random_forest, test, type = "prob")[, 2]

# Threshold the predicted probabilities
threshold <- 0.5
random_forest_predictions_threshold <- ifelse(random_forest_predictions > threshold, TRUE, FALSE)

# Evaluate the model's performance
random_forest_metrics <- get_metrics(random_forest_predictions_threshold, test$has_card, "Random Forest")
...
make_roc_plot_and_get_auc(random_forest_predictions_threshold, test$has_card)
...
# Train the model
xg_boost <- xgboost(data = train_preds, label = train_labels, nrounds = 1000, objective = "binary:logistic", verbose = 0, maximize = "F1", params = param)

print(xg_boost)

# Make predictions on the test data
xg_boost_predictions <- predict(xg_boost, test_preds)

# Threshold the predicted probabilities
threshold <- 0.5
xg_boost_predictions_threshold <- ifelse(xg_boost_predictions > threshold, TRUE, FALSE)

# Evaluate the model's performance
xg_boost_metrics <- get_metrics(xg_boost_predictions_threshold, test_labels, "XGBoost")
