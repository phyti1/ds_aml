...
```{r}
print(paste("Die optimale Anzahl von Parametern ist: ", len(predictors(rfe_dt))))
...
for(i in 1:length(val_sets)) {
  ...
  decision_tree_rfe <- rpart(has_card ~ ., data = train_sets[[i]] %>% select(predictors(rfe_dt)), method = "class")
...
```{r}
print(paste("Die optimale Anzahl von Parametern ist: ", len(predictors(rfe_rf))))
print("Die Parameter sind:")
...
for(i in 1:length(val_sets)) {
  ...
  random_forest_rfe <- randomForest(has_card ~ ., data = train_sets[[i]] %>% select(predictors(rfe_rf)), method = "class")
...
for(i in 1:length(val_sets)) {
  ...
  random_forest_tuned <- randomForest(has_card ~ ., data = train_sets[[i]] %>% select(predictors(rfe_rf)), method = "class", control = control)
...
train_data <- train_sets[[i]]
  ...
  test_preds <- as.matrix(sapply(test_data[,-1], as.numeric))
  test_labels <- as.factor(test_data[, 1])
