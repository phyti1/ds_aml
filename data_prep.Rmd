...
Betrachten wir nur die Falschklassifikationen der Top-3 Vorhersagen, sehen wir, dass der XGBoost mit 0 falschen Klassifizierungen bemerkenswert abschneidet. Auf den Top-10 sind Decision Tree, RandomForest und XGBoost sehr ähnlich. Die Regression ist hingegen sehr konstant: auf den Top-7 vorhersagen hat er nur bei jeweils einem Validierungsset unrecht behalten. 
...
Die Confusion Matritzen der Modelle Random Forest, Decision Tree und XGBoost sind sich sehr ähnlich. Einzig die Regression fällt wieder ab.
...
Die ROC Kurve sollte sich möglichst weit von der grauen Linie entfernen, dies tut sie auch bei allen Modellen recht gut. Die AUC's der noch relevanten Modelle sind alle über einem Wert von 0.83. Random Forest und XGBoost sind hier sehr ähnlich, Decision Tree ein wenig schlechter. Dies liegt aber auch daran, dass beim Decision Tree eine nicht so schöne Kurve entstanden ist. 
...
Bis jetzt haben wir alle Modelle mit Off-Time-Daten evaluiert. Zum Schluss möchen wir noch schauen, ob unsere Metriken bei zufälligen Testdaten besser werden würden. Dafür müssen wir einen neuen Train-Test-Split erstellen. Unserer Erwartung ist, dass die Scores ein wenig höher werden.
...
Wir hatten recht: Die Metriken werden höher. Vor allem die Precision profitiert von dem anderen Train-Test-Split. Aber auch wennn diese Scores hier höher sind, ist die Off-Time Variante die sinnvollere für eine reale Anwendung. Es müssen immer Zeitpunkte in der Zukunft vorhergesagt werden. 
