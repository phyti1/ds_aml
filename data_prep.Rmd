...
get_cv_scores <- function(train_routine)
{
  predictions_list <- c()
  # Fit the model on the training data
  for(i in 1:length(val_sets))
  {
    predictions_threshold <- train_routine(train_sets[[i]], val_sets[[i]])
    predictions_list[[i]] <- predictions_threshold
  }
  # refit on full training dataset
  predictions_threshold <- train_glm(train, test)

  return(get_metrics_mean(predictions_list, val_sets))
}

get_cv_scores(train_glm)
...
```{r}
predictions_threshold <- train_glm(train %>% select(max_difference_12, max_difference_11, has_card), test)
...
train_xgb <- function(train_df, val_df)
{
  ...
  # Train the model
  model <- xgboost(data = as.double(train_preds), label = train_labels, nrounds = 1000, objective = "binary:logistic", verbose = 0, maximize = "F1", params = param)
...
model <- train(x = train_preds, y = train_labels, method = "xgbTree", trControl = trainControl(method = "cv", number = 5), tuneGrid = param_grid, metric = "F", verbose = FALSE, verbosity = 0)
