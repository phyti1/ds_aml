...
Der Typ der Karte ist immer gleich.
...
An wen die Zahlungen gehen sollten keinen Einfluss auf unser Modell haben.
...
Zuerst überprüfen wir, ob es sich dabei immer um das selbe Gebiet handelt.

```{r}
sum(full$district_id != full$district_id.accounts)
```

409 Personen haben ihren Account also in einem anderen Distrikt, als sie zugewiesen sind.
Wir müssen also die District-Informationen für beide District-ID's zuweisen.
...
Die Transaktionen werden für Kreditkartenkäufer und Nicht-Käufer aggregiert, weshalb wir diese als ersten anhand der Variable "has_card" trennen.
...
Wir erstellen also für jeden Kreditkartenkäufer ein Rollup-Fenster für die letzten zwölf Monate vor dem Kauf der Kreditkare, minus einem Monat Lag.
Diesen Monat beachten wir nicht, da die Bearbeitung eines Kreditkartenkaufs etwa diese Zeitspanne dauert und sich der Kunde dementsprechend bereits vor dem Kaufdatum für eine Kreditkarte interessiert hat.
...
Dies liegt entweder daran, dass der Kunde nicht viele Transaktionen tätigt oder die Kreditkarte bereits im ersten Jahr gekauft hat.
...
Für jeden Kunden gibt es nun für jeden Monat vor dem Kreditkartenkauf eine Row mit den aggregierten Daten des Monats.
...
Dafür benutzen wir die Funktion RFE vom Paket caret.
RFE steht für "Recursive Feature Elimination" (rekursive Merkmalsentfernung) und ist eine Technik, die verwendet wird, um eine Teilmenge der relevantesten Merkmale aus einem gegebenen Datensatz auszuwählen.
...
Wir möchten den Datensatz auf 10 Features reduzieren.
Dabei entfernen wir bei jedem Durchlauf 10 Features.
...
Um den Decision Tree noch weiter zu verbessern, wollen wir die verschiedenen Hyperparameter optimieren. Hyperparameteroptimierung (auch Tuning genannt) ist der Prozess, die beste Set von Hyperparametern für ein maschinelles Lernmodell zu finden.
Wir probieren, folgenden Parameter zu optimieren:

cp (complexity parameter): Der cp-Parameter legt fest, wie restriktiv der Baum beim Aufteilen von Knoten ist. Ein kleinerer Wert führt zu einem komplexeren Baum mit mehr Knoten und möglicherweise besserer Anpassungsfähigkeit. Ein größerer Wert führt zu einem einfacheren Baum mit weniger Knoten und möglicherweise besserer Generalisierbarkeit. 
Mögliche Werte: c(0.001, 0.01, 0.05, 0.1)

maxdepth : Dieser Parameter legt die maximale Tiefe des Baums fest. Ein größerer Wert ermöglicht es dem Baum, tiefer zu wachsen und möglicherweise bessere Anpassungsfähigkeit zu erreichen, während ein kleinerer Wert dazu führen kann, dass der Baum einfacher und generalisierbarer ist. 
Mögliche Werte: c(5,10,15)

minsize : Dieser Parameter legt die minimale Anzahl von Beobachtungen in einem Knoten fest, die erforderlich ist, damit dieser Knoten weiter aufgeteilt werden kann. Ein größerer Wert führt dazu, dass der Baum einfacher ist und weniger überanpassung erfährt, während ein kleinerer Wert dazu führen kann, dass der Baum komplexer ist und möglicherweise bessere Anpassungsfähigkeit erreicht. 
Mögliche Werte: c(2,5,10)

In Python würden wir dies normalerweise mit einem GridSearch implementieren. Hier hatten wir aber Probleme damit: Nur ein Paremeter hätte optimiert werden können (cp). Deshalb haben wir die verschiedenen Werte manuell ausprobiert und jeweils den Wert mit der höchsten Precision als bester Parameter genommen.
...
control = rpart.control(minsize = 1, cp = 0.01)

for(i in 1:length(val_sets)) {
  set.seed(61)
  
  decision_tree_tuned <- rpart(has_card ~ ., data = train_sets[[i]], method = "class", control = control)
...
colMeans(decision_tree_tuned_metrics %>% select(-model))
```

```{r}
make_roc_plot_and_get_auc(decision_tree_tuned_predictions[[id_set]], val_sets[[id_set]]$has_card)
```

Die Klassifikation ist um einiges besser auf dem Decision Tree.
Es scheint also für unseren Verwendungszweck der bessere Algorithmus zu sein.
Wir untersuchen noch Erweiterungen des Decision Trees: den Random Forest und XGBoost
...
Der Random Forest ist ein wenig besser als der Decision Tree.
Als nächstes wollen wir probieren, ob eine Hyperparameteroptimierung unser Resultat noch verbessern kann.
Bei der Hyperparameteroptimierung gehen wir wieder gleich vor, wie beim Decision Tree.
Wir probieren, folgende Parameter zu optimieren:
...
"gini" Unreinheit und "extratrees" sind zwei häufig verwendete Trennregeln.
min.node.size : die minimale Anzahl von Beobachtungen in einem Endknoten.
max.depth : die maximale Tiefe des Baumes.
Es ist eine Sequenz von Ganzzahlen von 1 bis 30 mit einem Schritt von 5.
ntree : die Anzahl der Bäume, die im Zufallswald gewachsen sind.

```{r}
param_grid <- expand.grid(mtry = c(0.1, 1, 10, 100), max = c(100))
...
print(random_forest_tuned$bestTune)
```

Auch hier nehmen wir wieder das optimierte Modell und untersuchen es mit unserer eigenen Kreuzvalidierung.
...
for(i in 1:length(val_sets)) {
  ...
  random_forest_tuned <- randomForest(has_card ~ ., data = train_sets[[i]], method = "class", mtry = 100, maxnodes = 100, ntree = 50)
...
(RandomForest liefert oft die gleichen Werte)

```{r}
users <- val_sets[[10]]

users$prediction <- decision_tree_predictions[[10]]
Decision_Tree <- head(users %>% arrange(desc(prediction)), 20)$has_card

users$prediction <- xg_boost_predictions[[10]]
XGBoost <- head(users %>% arrange(desc(prediction)), 20)$has_card


users$prediction <- regression_all_predictions[[10]]
Regression <- head(users %>% arrange(desc(prediction)), 20)$has_card


users$prediction <- random_forest_predictions[[10]]
RandomForest <- head(users %>% arrange(desc(prediction)), 20)$has_card

df <- data.frame(Regression, Decision_Tree, RandomForest, XGBoost)
...
users$prediction <- decision_tree_predictions[[1]]
Decision_Tree <- head(users %>% arrange(desc(prediction)), 20)$has_card

users$prediction <- xg_boost_predictions[[1]]
XGBoost <- head(users %>% arrange(desc(prediction)), 20)$has_card


users$prediction <- regression_all_predictions[[1]]
Regression <- head(users %>% arrange(desc(prediction)), 20)$has_card


users$prediction <- random_forest_predictions[[1]]
RandomForest <- head(users %>% arrange(desc(prediction)), 20)$has_card
...
```{r}
plot_confusion_matrix(regression_all_predictions_threshold[[7]], val_sets[[7]]$has_card, "Regression")
plot_confusion_matrix(decision_tree_predictions_threshold[[7]], val_sets[[7]]$has_card, "Decision Tree")
plot_confusion_matrix(random_forest_predictions_threshold[[7]], val_sets[[7]]$has_card, "Random Forest")
plot_confusion_matrix(xg_boost_predictions_threshold[[7]], val_sets[[7]]$has_card, "XGBoost")
```
...
make_roc_plot_and_get_auc_list <- function(predictions, true_values)
{
  ...
  legend("bottomright", legend=c("Logistic Regression", "Decision Tree", "Random Forest", "XGBoost"), col=colors, pch=4)
}


make_roc_plot_and_get_auc_list(list(regression_baseline_predictions[[7]], decision_tree_tuned_predictions[[7]], random_forest_tuned_predictions[[7]], xg_boost_predictions[[7]]), val_sets[[7]]$has_card)
